...
**How the pilot runs — "the app IS the pilot" (instrumented soft-launch).** We do NOT build a separate pilot; the launched MVP collects the data. But it must be *instrumented*, not vibes-only, because the two spikes above resolve on STATISTICS, not on «нравится»-feedback:
- **Product signals (in-app, continuous):** "это не про меня" per card; a 1-tap «точно про меня / общо» on the result (anti-Barnum); share-rate; completion/drop-off; follow-up reply. → iterate the product.
- **Psychometric validity (one statistical pass at ~150–200 responses):** `/submit` already stores answers (encrypted). Missing piece = an export of pseudonymized answers + an α/EFA/correlation script. This is the only thing that can tell us if the typology holds — no amount of positive feedback substitutes for it. Guards against shipping a measurement-invalid horoscope that "feels" fine (Barnum) to the exact rational audience that bounces on it.
- **Claims stay humble** (probabilistic wording, already built) until that pass; then finalize items / calibrate / pivot to plan Б.
- Implementation requirement (Ф3/Ф4): capture the product signals in the UI + persist (no PII); one small task for the export + analysis harness.
